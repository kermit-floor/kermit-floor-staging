
import { Header } from '@/components/showcase/Header';
import { Showcase } from '@/components/showcase/Showcase';
import { Chatbox } from '@/components/showcase/Chatbox';
import { Footer } from '@/components/showcase/Footer';
import { getSkirtingXLine100mm } from '@/lib/skirting-x-line-100-mm-data';
import { getTranslations } from 'next-intl/server';
import type { Metadata } from 'next';
import { getAlternatesForRoute, getCanonicalForRoute } from '@/lib/seo/canonical';
import { CollectionJsonLd } from '@/components/seo/CollectionJsonLd';

export async function generateMetadata({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'SkirtingPages' });
 
  return {
    title: t('x-line-100-mm.title'),
    description: t('x-line-100-mm.description'),
    alternates: getAlternatesForRoute('/spc-skirting-boards/x-line-100-mm-skirting-board', locale),
    openGraph: {
      title: t('x-line-100-mm.title'),
      description: t('x-line-100-mm.description'),
      url: getCanonicalForRoute('/spc-skirting-boards/x-line-100-mm-skirting-board', locale),
    },
  };
}

export default async function SkirtingXLine100mmPage({params}: {params: Promise<{ locale: string }>}) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'SkirtingPages' });
  const panels = await getSkirtingXLine100mm();

  return (
    <main className="min-h-screen flex flex-col bg-background">
      <Header pageType="skirting-x-line-100-mm" />
      <div className="flex-grow">
        <Showcase initialPanels={panels} collectionType="skirting-x-line-100-mm" />
      </div>
      <Footer />
      <Chatbox />
      <CollectionJsonLd
        locale={locale}
        collectionKey="skirting-x-line-100-mm"
        kind="product"
        description={t('x-line-100-mm.description')}
        panels={panels}
      />
    </main>
  );
}

    
